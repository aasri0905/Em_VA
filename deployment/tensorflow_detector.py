...
class TensorflowDetector(object):
    ...
    def run(self, image):
        ...
        if len(images_for_prediction) > 0:
            start_time = time.time()
            prediction = self.sess_2.run(classification_output,feed_dict={classification_input: images_for_prediction})
            ...
            for row in prediction:
                ...
                emotions_score.append(round(row[pred], 2))
            print('--------------------------------------')
            start_time = time.time()
            ...
            for row in prediction:
                ...
                valence__.append(round(row[0], 5))
                arousal__.append(round(row[1], 5))
                
            print('\n')

        return boxes, scores, classes, num_detections, emotions_detected,valence__,arousal__,emotions_score
